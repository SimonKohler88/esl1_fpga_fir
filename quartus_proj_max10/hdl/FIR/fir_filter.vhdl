...
port (
    ...
    Av_S_DATA_IN: in std_logic_vector(15 downto 0);
    Av_S_DATA_IN_valid: in std_logic;
    Av_S_DATA_IN_ready: out std_logic := '0';

    Av_S_DATA_OUT: out  std_logic_vector(15 downto 0) := "0000000000000000";
    Av_S_DATA_OUT_valid: out std_logic;
    Av_S_DATA_OUT_ready: in std_logic;
...
architecture FIR_Filter_Arch of FIR_Filter is
    ...
    -- Wait for rising edge on Av_S_DATA_IN_valid
...
begin
    fsclk <= Av_S_DATA_IN_valid;
    data_i <= Av_S_DATA_IN;

    Av_S_DATA_OUT_valid <= valid_out;
    ...
    if rising_edge(clk) then
        ...
        when idle_st =>
            if fsclk = '1' and fsclk_q = '0' then
                state <= active_st;
                Av_S_DATA_IN_ready <= '0';
            else
                Av_S_DATA_IN_ready <= '1';
            end if;
        ...
        when ready_out_st =>
            if Av_S_DATA_OUT_ready='1' and valid_out='0' then
                state <= idle_st;
                valid_out <= '1';
            else
                valid_out <= '0';
        ...
        if counter=0 and state=active_st then
            Av_S_DATA_OUT <= std_logic_vector(
                signed(
                    output(
